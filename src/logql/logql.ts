...
const vector_matching = [
	'by',
	'without',
];
// Produce a regex matching elements : (by|without)
const vectorMatchingRegex = `(${vector_matching.reduce((prev, curr) => `${prev}|${curr}`)})`;
...
const keywords = aggregations.concat(parsers).concat(format_expressions).concat(vector_aggregations).concat(vector_matching);
